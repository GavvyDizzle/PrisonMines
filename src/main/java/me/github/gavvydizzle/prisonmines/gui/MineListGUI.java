...
import me.github.gavvydizzle.prisonmines.mines.MineIdSorter;
...
public class MineListGUI implements ClickableMenu {
    ...
    public void handleClick(InventoryClickEvent e) {
        ...
        if (e.getSlot() == pageUpSlot) {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(PrisonMines.getInstance(), () -> {
                inventoryManager.openMineGUI((Player) e.getWhoClicked(), mine);
    ...
    private void updatePage(Player player) {
        ...
        for (int i = 0; i < getNumItemsOnPage(page); i++) {
            inventory.setItem(i, mineItems.get(getNumItemsOnPage(page)));
        }
